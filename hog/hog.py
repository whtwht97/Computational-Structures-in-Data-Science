...
def is_swap(player_score, opponent_score):
    ...
    firstdigit = first_digit(excitement)
    if excitement % 10 == firstdigit:
        return True
    else:
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            turn_outcome = take_turn(strategy0(score0,score1),score1,dice)
            if feral_hogs:
                if abs(prev_score0 - strategy0(score0,score1)) == 2:
                    prev_score0 = turn_outcome
                    turn_outcome = turn_outcome + 3
                else:
                    prev_score0 = turn_outcome
            score0 = score0 + turn_outcome
            if is_swap(score0,score1):
                score0,score1=score1,score0
        else:
            turn_outcome = take_turn(strategy1(score1,score0),score0,dice)
            if feral_hogs:
                if abs(prev_score1 - strategy1(score1,score0)) == 2:
                    prev_score1 = turn_outcome
                    turn_outcome = turn_outcome + 3
                else:
                    prev_score1 = turn_outcome
            score1 += turn_outcome
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def commentary(score0,score1):
        ...
        if curr_score - prev_score > prev_high:
            ...
            return announce_highest(who,prev_high=curr_score-prev_score,prev_score=curr_score)
        else:
            return announce_highest(who,prev_high=prev_high,prev_score=curr_score)
    return commentary
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while turn <= 10:
        if max_avg_score <= averaged_dice(turn,dice):
            max_avg_score = averaged_dice(turn,dice)
            rolls = turn
        turn += 1
    return rolls
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    if is_swap(new_score,opponent_score) and new_score < opponent_score:
        return 0
    elif new_score == opponent_score and free_bacon(opponent_score) <= margin:
        return num_rolls
